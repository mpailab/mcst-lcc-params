...
/* Получить значение характеристики процедуры */
#define ann_GetProcChar( proc_chars, proc_char) \
    arr_GetProf( (proc_chars), (arr_Index_t)(proc_char))

/* Установить значение характеристики процедуры */
#define ann_SetProcChar( proc_chars, proc_char, value) \
{ \
    arr_SetProf( (proc_chars), (arr_Index_t)(proc_char), (value)); \
}
 ...
 */
void
ann_PrintProcChars( ire2k_Proc_ref proc ) /* процедура */
{
    arr_Array_ptr proc_chars = ann_CalcProcChars( proc);
    buff_Buffer_ptr buff_p;
    ...
    for ANN_ALL_PROC_CHARS( proc_char)
    {
        ann_PrintProf( buff_p, "#", ann_GetProcChar( proc_chars, proc_char));
    }
