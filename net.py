...
def create_net(procs_dic, pgroup, points_num = gl.points_num):
    ...
    for parname in pgroup:
        if parname == 'dcs':
            par_value_default['dcs_kill'] = par.defaults['dcs_kill']
            par_value_default['dcs_level'] = par.defaults['dcs_level']
        else:
            par_value_default[parname] = par.defaults[parname]
