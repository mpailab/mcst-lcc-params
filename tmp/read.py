...
STAT_PATH = './spec/stat/comp'
FULL_STAT_PATH = './spec/stat/comp_all/stat'
#STAT_PATH_FOR_READ = './spec/stat/tmp_comp'
...
def task_list():
    return listdir(STAT_PATH)

def proc_list(taskname, add_unexec_procs = USE_FULL_STAT):
    if add_unexec_procs:
        tpath = FULL_STAT_PATH + '/' + taskname
        list_procs = listdir(tpath)
    else:
        path = PROC_LIST_PATH + '/' + taskname + '.txt'
        ffile = open(path)
        list_procs = []
        for strr in ffile:
...
def proc_cnt_dic(taskname):
    ...
    if USE_FULL_STAT:
        for procname in proc_list(taskname):
            proc_weight_dir[procname] = UNEXEC_PROC_WEIGHT

    ffile = open(PROC_ORDER_PATH + '/' + taskname + '.txt')
    for string in ffile:
...
def task_cnt(taskname, num = 1):
    path = STATEXEC_PATH + '/' + taskname + '/' + 'run.' + str(num) + '.txt'
    ffile = open(path)
